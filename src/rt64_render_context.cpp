// RT64 Render Context for Extreme-G
...
class ExtremeGRenderContext : public ultramodern::renderer::RendererContext {
public:
    ExtremeGRenderContext(uint8_t* rdram, ultramodern::renderer::WindowHandle window_handle, bool developer_mode)
        : rdram_(rdram), window_handle_(window_handle), developer_mode_(developer_mode)
    {
        ...
        case RT64::Application::SetupResult::Success:
            printf("[ExtremeG] RT64 initialized successfully\n");
            is_valid_ = true;
            break;
        case RT64::Application::SetupResult::DynamicLibrariesNotFound:
            fprintf(stderr, "[ExtremeG] RT64: Dynamic libraries not found\n");
            setup_result = ultramodern::renderer::SetupResult::DynamicLibrariesNotFound;
            break;
        case RT64::Application::SetupResult::InvalidGraphicsAPI:
            fprintf(stderr, "[ExtremeG] RT64: Invalid graphics API\n");
            setup_result = ultramodern::renderer::SetupResult::InvalidGraphicsAPI;
            break;
        case RT64::Application::SetupResult::GraphicsAPINotFound:
            fprintf(stderr, "[ExtremeG] RT64: Graphics API not found\n");
            setup_result = ultramodern::renderer::SetupResult::GraphicsAPINotFound;
            break;
        case RT64::Application::SetupResult::GraphicsDeviceNotFound:
            fprintf(stderr, "[ExtremeG] RT64: Graphics device not found\n");
            setup_result = ultramodern::renderer::SetupResult::GraphicsDeviceNotFound;
            break;
        }

        if (!is_valid_) {
            printf("[ExtremeG] RT64 setup failed, destroying app\n");
            app_.reset();
        }
    }

    ~ExtremeGRenderContext() override {
        if (app_) {
            printf("[ExtremeG] Destroying RT64 application\n");
            app_.reset();
        }
    }
    ...
    void send_dl(const OSTask* task) override {
        ...
        if (dl_count_ <= 5 || (dl_count_ % 60 == 0)) {
            printf("[ExtremeG] Display list #%u at 0x%08X\n", dl_count_, dl_address);
        }
    ...
    void update_screen() override {
        ...
        if (screen_count_ <= 5 || (screen_count_ % 60 == 0)) {
            printf("[ExtremeG] update_screen #%u\n", screen_count_);
        }
        app_->updateScreen();
    }

    void shutdown() override {
        printf("[ExtremeG] Render context shutdown\n");
        if (app_) {
            app_.reset();
        }
        is_valid_ = false;
...
// Factory function
std::unique_ptr<ultramodern::renderer::RendererContext> create_extremeg_render_context(
    uint8_t* rdram, ultramodern::renderer::WindowHandle window_handle, bool developer_mode) {
    return std::make_unique<ExtremeGRenderContext>(rdram, window_handle, developer_mode);
}
